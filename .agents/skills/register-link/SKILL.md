---
name: register-link
description: Generates signed, expiring registration links (/register) for usbasketball.nl via scripts/generate-registration-link.mjs — defaults to production, or targets a Vercel preview deployment / local dev when asked. Use when the user asks for a registration URL, preview registration link, or expiring register link.
---

# Register Link

Generate signed, expiring access links for the private registration page (`/register`) of usbasketball.nl. Links are HMAC-signed by `scripts/generate-registration-link.mjs`, which reads `REGISTRATION_SECRET` from `.env.local` automatically.

## Step 1 — Determine the base URL

Default to production: `https://www.usbasketball.nl`. Only deviate from it when the user asks for something else. Resolve in this order:
...
2. **Local dev requested**: If the user asks for local development, use `http://localhost:3000`.
3. **Preview requested**: If the user asks for a preview/development deployment, or names a branch or PR instead, find the latest Vercel preview URL for it:
...
4. **Ask**: If you still cannot determine the preview URL, stop and ask the user to paste it from the PR checks or the Vercel dashboard. Do not guess a vercel.app URL.
5. **Otherwise**: Use `https://www.usbasketball.nl`.
